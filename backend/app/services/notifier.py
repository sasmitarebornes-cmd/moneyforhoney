...
class MultiChannelNotifier:
    ...
    async def _send_single_telegram(self, client: Any, chat_id: str, text: str) -> bool:
        """Helper to send a message to a single Telegram target (Chat ID or @channel_username)."""
        if not httpx or client is None:
            logger.info(
                "Telegram dispatch (offline/no httpx): %s -> %s...", chat_id, text[:60]
            )
            return True
    ...
    async def send_telegram_message(
        ...
        if not httpx:
            logger.info("[NO HTTPX INSTALLED] Simulated dispatch: %s", text[:80])
            return True
    ...
    async def notify_profit_harvest(
        ...
        symbol_header = ""
        if exit_price > 0:
            symbol_header = f"🎯 <b>Target Hit:</b> #{symbol.replace('/', '')} at <code>${exit_price:,.4f}</code>\n"

        msg = (
            "🎯 <b>TAKE PROFIT & PROFIT WATERFALL HARVEST</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{symbol_header}"
...
__all__ = ["MultiChannelNotifier", "notifier"]
